...
render_status_table <- function(df) {
  ...
  info_header <- function(label, tooltip) {
    htmltools::HTML(
      sprintf(
  ...
  reactable::reactable(
    ...
    columns = list(
      ...
      dev_branch_status = reactable::colDef(
        header = info_header(
          "Dev status",
          "The difference in commits between 'main' and 'dev' branches: A positive number means dev is ahead of main by n commits; A negative number means dev is behind main by n commits; \"Unavailable\" means that there is no branch named 'dev' and/or 'main'."
        ),
        html = TRUE,
        minWidth = 160
      ),
